...
这种方式在开始是很难做到的，但是通过多次可以练习，形成这种行为习惯，内化成思维习惯，也是益处多多的，非常建议你去刻意练习。
...
### 分离关注点
TDD的方式，写测试代码时，我们只关注功能验收，而且只关注Tasking出来一个Task（业务场景）。写功能代码时，只用关注如何本次验收，即让当前的测试用例通过。测试通过后，你只用关注代码设计上，要不要将代码从刚刚可用的状态重构成简洁可用的状态。这个过程很好分离了我们的关注点，降低思维的负担，帮助专注聚焦，有效地提升开发效率。
...
### 三条童子军规
在美国童子军中，流传一条这样的简的军规：`让营地比你来时更干净`。Bob大叔在他的《代码整洁之道》中也引用这个词，提倡持续改进 -- 作为一个专业高素养的程序员，要尽全力让项目代码随着时间流逝而越变越好。那么借用这个说法，在TDD中，我们也提倡三条童子军规：
...
- 违反第二条，如果写了多个测试，测试长时间没法通过，会增加开发者的心里压力。实际上，伴随着知识的积累，业务细节的深入，Task会发生变更，测试会随之修改，那之前写的测试就成了包袱。
